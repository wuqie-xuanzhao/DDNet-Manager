...
fn common_scan_roots_include_user_appdata_program_and_steam_locations() {
    ...
    assert!(roots.contains(&std::path::Path::new("C:/Users/Player/AppData/Local").to_path_buf()));
    assert!(roots.contains(&std::path::Path::new("C:/Users/Player/AppData/Roaming").to_path_buf()));
    assert!(roots.contains(
        &std::path::Path::new("C:/ProgramData/Microsoft/Windows/Start Menu/Programs").to_path_buf()
    ));
    assert!(roots
        .contains(&std::path::Path::new("F:/SteamLibrary/steamapps/common/DDNet").to_path_buf()));
}
