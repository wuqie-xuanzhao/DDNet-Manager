...
pub fn extract_zip_to_staging(zip_path: &Path, staging_dir: &Path) -> Result<(), ManagerError> {
    let staging_root = prepare_staging_dir(staging_dir)?;

    let zip_file = fs::File::open(zip_path)
        .map_err(|error| ManagerError::Internal(format!("failed to open zip file: {error}")))?;
    let mut archive = zip::ZipArchive::new(zip_file)
        .map_err(|error| ManagerError::Internal(format!("invalid zip file: {error}")))?;
...
fn collect_zip_entries(
    ...
    staging_root: &Path,
) -> Result<Vec<(usize, PathBuf)>, ManagerError> {
    let mut file_entries: Vec<(usize, PathBuf)> = Vec::new();
    let mut unpacked_bytes = 0_u64;
    for index in 0..archive.len() {
        let entry = archive.by_index(index).map_err(|error| {
            ManagerError::Internal(format!("failed to read zip entry: {error}"))
        })?;
        ...
        let enclosed_name = entry.enclosed_name().ok_or_else(|| {
            ManagerError::Internal(format!("unsafe zip entry path: {}", entry.name()))
        })?;
        ...
        if entry.is_dir() {
            fs::create_dir_all(&output_path).map_err(|error| {
                ManagerError::Internal(format!("failed to create zip directory: {error}"))
            })?;
            continue;
        }
        ...
        unpacked_bytes = unpacked_bytes
            ...
            .ok_or_else(|| ManagerError::Internal("zip unpacked size overflow".to_string()))?;
        ...
        if let Some(parent) = output_path.parent() {
            fs::create_dir_all(parent).map_err(|error| {
                ManagerError::Internal(format!("failed to create zip parent directory: {error}"))
            })?;
        }
...
fn extract_zip_entries_serial(
    ...
    for (index, output_path) in file_entries.into_iter() {
        ...
        let mut output = fs::File::create(&output_path).map_err(|error| {
            ManagerError::Internal(format!("failed to create extracted file: {error}"))
        })?;
...
fn extract_zip_entries_parallel(
    ...
    std::thread::scope(|s| {
        let handles: Vec<_> = file_entries
            .chunks(chunk_size)
            .map(|chunk| {
                s.spawn(move || -> Result<(), ManagerError> {
                    let thread_file = fs::File::open(zip_path).map_err(|error| {
                        ManagerError::Internal(format!(
                            "failed to reopen zip file in worker: {error}"
                        ))
                    })?;
                    let mut thread_archive =
                        zip::ZipArchive::new(thread_file).map_err(|error| {
                            ManagerError::Internal(format!("invalid zip file in worker: {error}"))
                        })?;
                    for (index, output_path) in chunk {
                        let mut entry = thread_archive.by_index(*index).map_err(|error| {
                            ManagerError::Internal(format!("failed to read zip entry: {error}"))
                        })?;
                        let mut output = fs::File::create(output_path).map_err(|error| {
                            ManagerError::Internal(format!(
        ...
        for handle in handles {
            handle.join().map_err(|error| {
                ManagerError::Internal(format!("zip worker thread panicked: {error:?}"))
            })??;
        }
        Ok(())
    })
...
pub fn extract_tar_xz_to_staging(
    tar_xz_path: &Path,
    staging_dir: &Path,
) -> Result<(), ManagerError> {
    let staging_root = prepare_staging_dir(staging_dir)?;
    let file = fs::File::open(tar_xz_path)
        .map_err(|error| ManagerError::Internal(format!("failed to open tar.xz file: {error}")))?;
    ...
    let entries = archive
        .entries()
        .map_err(|error| ManagerError::Internal(format!("invalid tar.xz file: {error}")))?;
    for entry in entries {
        let mut entry = entry.map_err(|error| {
            ManagerError::Internal(format!("failed to read tar entry: {error}"))
        })?;
        unpacked_files = unpacked_files
            .checked_add(1)
            .ok_or_else(|| ManagerError::Internal("tar entry count overflow".to_string()))?;
        if unpacked_files > MAX_ZIP_FILES {
            return Err(ManagerError::Internal(format!(
                "tar contains more than {MAX_ZIP_FILES} files"
            )));
        }
...
fn extract_dmg_to_staging(dmg_path: &Path, staging_dir: &Path) -> Result<(), ManagerError> {
    #[cfg(target_os = "macos")]
    {
...
fn prepare_staging_dir(staging_dir: &Path) -> Result<PathBuf, ManagerError> {
    if staging_dir.exists() {
        fs::remove_dir_all(staging_dir).map_err(|error| {
            ManagerError::Internal(format!("failed to clear staging dir: {error}"))
        })?;
    }
    fs::create_dir_all(staging_dir).map_err(|error| {
        ManagerError::Internal(format!("failed to create staging dir: {error}"))
    })?;
    fs::canonicalize(staging_dir).map_err(|error| {
        ManagerError::Internal(format!("failed to canonicalize staging dir: {error}"))
    })
}
...
fn prepare_tar_entry<R: Read>(
    ...
    let entry_path = entry
        .path()
        .map_err(|error| ManagerError::Internal(format!("failed to read tar entry path: {error}")))?
        .into_owned();
    ...
    ensure_inside_root(staging_root, &output_path).map_err(|_| {
        ManagerError::Internal(format!("unsafe tar entry path: {}", entry_path.display()))
    })?;
    ...
    if entry_type.is_dir() {
        fs::create_dir_all(&output_path).map_err(|error| {
            ManagerError::Internal(format!("failed to create tar directory: {error}"))
        })?;
        return Ok(None);
    }
...
fn add_tar_entry_size<R: Read>(
    ...
    let entry_size = entry.header().size().map_err(|error| {
        ManagerError::Internal(format!("failed to read tar entry size: {error}"))
    })?;
    current_size
        .checked_add(entry_size)
        .ok_or_else(|| ManagerError::Internal("tar unpacked size overflow".to_string()))
}
...
fn extract_tar_file_entry<R: Read>(
    ...
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            ManagerError::Internal(format!("failed to create tar parent directory: {error}"))
        })?;
    }
    let mut output = fs::File::create(output_path).map_err(|error| {
        ManagerError::Internal(format!("failed to create extracted file: {error}"))
    ...
    std::io::copy(entry, &mut output)
        .map_err(|error| ManagerError::Internal(format!("failed to extract tar entry: {error}")))?;
    ...
    #[cfg(unix)]
    {
        ...
        fs::set_permissions(output_path, fs::Permissions::from_mode(mode)).map_err(|error| {
            ManagerError::Internal(format!("failed to set extracted file permissions: {error}"))
        })?;
    }
...
#[cfg(target_os = "macos")]
fn extract_dmg_to_staging_macos(dmg_path: &Path, staging_dir: &Path) -> Result<(), ManagerError> {
    ...
    if mount_dir.exists() {
        fs::remove_dir_all(&mount_dir).map_err(|error| {
            ManagerError::Internal(format!("failed to clear dmg mount dir: {error}"))
        })?;
    }
    fs::create_dir_all(&mount_dir).map_err(|error| {
        ManagerError::Internal(format!("failed to create dmg mount dir: {error}"))
    })?;
    ...
    let attach_status = Command::new("hdiutil")
        ...
        .arg("-readonly")
        .status()
        .map_err(|error| ManagerError::Internal(format!("failed to attach dmg: {error}")))?;
    if !attach_status.success() {
        let _ = fs::remove_dir_all(&mount_dir);
        return Err(ManagerError::Internal(format!(
            "failed to attach dmg: {attach_status}"
        )));
    }

    let copy_result = find_first_app_bundle(&mount_dir).and_then(|app_bundle| {
        let bundle_name = app_bundle.file_name().ok_or_else(|| {
            ManagerError::Internal("dmg app bundle has no directory name".to_string())
        })?;
    ...
    let detach_result = Command::new("hdiutil")
        ...
        .arg("-quiet")
        .status()
        .map_err(|error| ManagerError::Internal(format!("failed to detach dmg: {error}")));
    let _ = fs::remove_dir_all(&mount_dir);

    match (copy_result, detach_result) {
        (Ok(()), Ok(status)) if status.success() => Ok(()),
        (Ok(()), Ok(status)) => Err(ManagerError::Internal(format!(
            "failed to detach dmg: {status}"
        ))),
        (Err(error), _) => Err(ManagerError::Internal(format!(
            "failed to copy app bundle from dmg: {error}"
        ))),
...
#[cfg(target_os = "macos")]
fn find_first_app_bundle(root: &Path) -> Result<PathBuf, ManagerError> {
    let entries = fs::read_dir(root).map_err(|error| {
        ManagerError::Internal(format!("failed to read dmg mount dir: {error}"))
    })?;
    for entry in entries {
        let path = entry
            .map_err(|error| {
                ManagerError::Internal(format!("failed to read dmg mount entry: {error}"))
            })?
            .path();
        if path
            .extension()
    ...
    Err(ManagerError::Internal(
        "dmg does not contain an app bundle".to_string(),
    ))
}

/// 在 staging 目录内寻找解压后的完整客户端根目录。
pub fn find_staged_client_dir(staging_dir: &Path) -> Result<PathBuf, ManagerError> {
    ...
    let entries = fs::read_dir(staging_dir)
        .map_err(|error| ManagerError::Internal(format!("failed to read staging dir: {error}")))?;
    for entry in entries {
        let path = entry
            .map_err(|error| {
    ...
    Err(ManagerError::Internal(
        "staging directory does not contain a healthy DDNet client".to_string(),
    ))
}

fn ensure_inside_root(root: &Path, path: &Path) -> Result<(), ManagerError> {
...
fn normalize_existing_or_parent(path: &Path) -> Result<PathBuf, ManagerError> {
    if path.exists() {
        return fs::canonicalize(path).map_err(|error| {
            ManagerError::Internal(format!("failed to canonicalize path: {error}"))
        });
    }

    let parent = path
        .parent()
        .ok_or_else(|| ManagerError::Internal(format!("path has no parent: {}", path.display())))?;
    normalize_existing_or_parent(parent)
}

fn copy_zip_entry<R: Read, W: Write>(reader: &mut R, writer: &mut W) -> Result<(), ManagerError> {
    std::io::copy(reader, writer)
        .map(|_| ())
        .map_err(|error| ManagerError::Internal(format!("failed to extract zip entry: {error}")))
}
