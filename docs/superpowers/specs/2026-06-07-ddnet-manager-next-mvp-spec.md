...
当前代码已经不是首版 mock：后端已拆出客户端扫描、SQLite 注册表、manifest 校验、下载任务、zip staging、安装回滚、网络路由和进程启动检测；前端也已有启动页、全部游戏、更新、设置页和集中 IPC 封装。
...
Workshop、Binds、资源管理、cfg 分析和 cfg 写入等功能不进入 0.1.0 可发布范围。当前代码已移除这些后端模块、共享类型和前端入口，后续如需重启必须重新写独立规格和实现计划。
...
- `src-tauri/src/models.rs` 已包含客户端安装、扫描选项、网络路由、更新检查、下载任务和 manifest 类型。
...
- `src-tauri/src/commands.rs` 已暴露客户端管理、启动、运行检测、manifest 加载、更新检查、下载和安装命令。
...
- `src/types.ts` 已同步当前启动器、客户端管理、更新和下载 IPC 类型。
...
- `src/App.tsx` 已形成主应用壳、启动页、游戏列表、更新和设置入口。
...
`BindInstallPlan`、资源路径 resolver、Workshop adapter 和 cfg writer 不属于 0.1.0 发布范围。当前仓库不再保留这些后端模块、共享类型或前端入口。后续重启这些能力时，必须重新写独立规格和实现计划。
...
### 5. 资源、Binds 与 Workshop 排除策略

0.1.0 不发布资源管理、Binds、Workshop 或 cfg 写入。当前仓库应保持无相关后端模块、共享类型或前端入口。

发布要求：

- 导航中不出现资源、Binds 或 Workshop 入口。
- 不允许在 0.1.0 中写入 settings、autoexec、Manager 专用 cfg 或任何用户资源目录。
- 不允许把 cfg 分析、Workshop 列表、bind 安装或裸 JSON 调试输出作为 0.1.0 功能页面。
- 如后续重启这些能力，必须先产出独立规格、风险边界和验证计划。
...
0.1.0 不发布资源页与 Binds 页，也不保留相关导航入口。
...
资源、Binds、Workshop、cfg 分析和 cfg 写入曾有局部代码，但已从当前范围实现中移除，0.1.0 不承诺这些功能。发布前必须检查导航、按钮、IPC 调用和文案，避免用户触达范围外流程；如后续重启，必须先补独立规格、风险边界和验证计划。
